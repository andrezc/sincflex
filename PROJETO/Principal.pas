...
begin
  try
    ...
    Query_Local.Close;
    Query_Local.SQL.Clear;
    Query_Local.SQL.Add('Select np.descricao as NM_Produto, coalesce(g.cod_barras, p.cod_barras) as COD_BARRAS, P.*,');
    ...
    Query_Local.SQL.Add('left outer join sp_nome_produto (p.cd_produto, g.cd_grade, p.nm_produto, '''', p.DESCRICAO_ABREV, ''F'', ''F'', '''', ''F'') as');
    Query_Local.SQL.Add('NP on 1 = 1');
